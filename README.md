...
* 语音识别:选用低成本离线语音芯片(如 LD3320),支持中文指令识别;
...
* 依赖库:STM32 标准外设库(STM32F1xx\_StdPeriph\_Lib)、LD3320 语音识别驱动库;
* 固件支持:支持通过 USB-TTL 模块烧录,支持指令自定义配置。

# 功能特点

1. **低成本易复现:** 核心元器件总价≤50 元,PCB 设计为双层板,焊接难度低,适合新手;
...
| ------------------| --------------------| -----------------------------------------------------------------| -----------|
|语音识别模块|LD3320|PA5(SPI\_SCK)、PA6(SPI\_MISO)、PA7(SPI\_MOSI)、PB0(CS)|SPI|
...
* 语音识别模块:LD3320 的 CS 引脚(片选)接 PB0,复位引脚(RST)接 PB1,低电平复位;
...
|区域|主要元器件布局|作用|
| ------------| ----------------------------------| -------------------------------|
|核心控制区|U1(STM32F103C8T6)、U2(LD3320)|MCU 与语音识别芯片,核心运算区|
...
2. 焊接顺序:先焊被动元器件(电阻、电容)→ 再焊芯片(STM32、LD3320、AMS1117)→ 最后焊接口(USB、端子座);
3. 焊接完成后,用万用表检测电源引脚(VCC-GND)是否短路,确认无误后接入 5V 电源(电源 LED 点亮表示供电正常)。

## 2. 软件烧录步骤

1. 安装开发环境:STM32CubeIDE 或 Keil MDK 5.37,导入项目源码(`Software`​ 文件夹下);
...
3. 故障排查:若唤醒无反应,检查 LD3320 接线(SPI 引脚、复位引脚);若指令不执行,检查 GPIO 与执行模块的接线。
